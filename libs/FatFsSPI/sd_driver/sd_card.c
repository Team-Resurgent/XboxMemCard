...
static bool sd_wait_ready(sd_card_t *pSD, int timeout) {
    ...
    if (resp == 0x00) DBG_PRINT("%s failed\r\n", __FUNCTION__);
...
static int sd_cmd(sd_card_t *pSD, const cmdSupported cmd, uint32_t arg,
    ...
    if (CMD12_STOP_TRANSMISSION != cmd) {
        if (false == sd_wait_ready(pSD, SD_COMMAND_TIMEOUT)) {
            DBG_PRINT("%s:%d: Card not ready yet\r\n", __FILE__, __LINE__);
        }
    }
    // Re-try command
    ...
    //	DBG_PRINT("CMD:%d \t arg:0x%" PRIx32 " \t Response:0x%" PRIx32 "\r\n",
    // cmd, arg, response);
    ...
    switch (cmd) {
        ...
        case CMD13_SEND_STATUS:  // Response R2
            ...
            response |= sd_spi_write(pSD, SPI_FILL_CHAR);
            if (response) {
                DBG_PRINT("R2: 0x%" PRIx32 "\r\n", response);
                if (response & 0x01 << 0) {
                    DBG_PRINT("Card is Locked                         \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_WRITE;
                }
                if (response & 0x01 << 1) {
                    DBG_PRINT("WP Erase Skip, Lock/Unlock Cmd Failed  \r\n");
                ...
                if (response & 0x01 << 4) {
                    DBG_PRINT("Card ECC Failed                        \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_WRITE;
                }
                if (response & 0x01 << 5) {
                    DBG_PRINT("WP Violation                           \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_WRITE_PROTECTED;
                }
                if (response & 0x01 << 6) {
                    DBG_PRINT("Erase Param                            \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_ERASE;
                }
                if (response & 0x01 << 7) {
                    DBG_PRINT("Out of Range, CSD_Overwrite            \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_PARAMETER;
                }
                if (response & 0x01 << 8) {
                    DBG_PRINT("In Idle State                          \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_NONE;
                }
                if (response & 0x01 << 9) {
                    DBG_PRINT("Erase Reset                            \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_ERASE;
                }
                if (response & 0x01 << 10) {
                ...
                if (response & 0x01 << 11) {
                    DBG_PRINT("Com CRC Error                          \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_CRC;
                }
                if (response & 0x01 << 12) {
                    DBG_PRINT("Erase Sequence Error                   \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_ERASE;
                }
                if (response & 0x01 << 13) {
                    DBG_PRINT("Address Error                          \r\n");
                    status = SD_BLOCK_DEVICE_ERROR_PARAMETER;
                }
                if (response & 0x01 << 14) {
...
static int sd_cmd8(sd_card_t *pSD) {
    ...
    if ((SD_BLOCK_DEVICE_ERROR_NONE == status) &&
        ...
        if ((response & 0xFFF) != arg) {
            DBG_PRINT("CMD8 Pattern mismatch 0x%" PRIx32 " : 0x%" PRIx32
                       "\r\n",
                       arg, response);
            pSD->card_type = CARD_UNKNOWN;
...
static uint64_t sd_sectors_nolock(sd_card_t *pSD) {
    ...
    if (sd_cmd(pSD, CMD9_SEND_CSD, 0x0, false, 0) != 0x0) {
        DBG_PRINT("Didn't get a response from the disk\r\n");
        return 0;
    }
    uint8_t csd[16];
    if (sd_read_bytes(pSD, csd, 16) != 0) {
        DBG_PRINT("Couldn't read csd response from disk\r\n");
        return 0;
    }
    ...
    switch (csd_structure) {
        case 0:
            ...
            DBG_PRINT("Standard Capacity: c_size: %" PRIu32 "\r\n", c_size);
            DBG_PRINT("Sectors: 0x%llx : %llu\r\n", blocks, blocks);
            DBG_PRINT("Capacity: 0x%llx : %llu MB\r\n", capacity,
        ...
        case 1:
            ...
            DBG_PRINT("SDHC/SDXC Card: hc_c_size: %" PRIu32 "\r\n", hc_c_size);
            DBG_PRINT("Sectors: %8llu\r\n", blocks);
            DBG_PRINT("Capacity: %8llu MB\r\n", (blocks / (2048U)));
            break;

        default:
            DBG_PRINT("CSD struct unsupported\r\n");
            return 0;
    };
    return blocks;
}
...
static bool sd_wait_token(sd_card_t *pSD, uint8_t token) {
    ...
    DBG_PRINT("sd_wait_token: timeout\r\n");
    return false;
}
...
static int sd_read_bytes(sd_card_t *pSD, uint8_t *buffer, uint32_t length) {
    ...
    if (crc_on) {
        ...
        if ((uint16_t)crc_result != crc) {
            DBG_PRINT("_read_bytes: Invalid CRC received 0x%" PRIx16
...
static int sd_read_block(sd_card_t *pSD, uint8_t *buffer, uint32_t length) {
    ...
    if (crc_on) {
        ...
        crc_result = crc16((void *)buffer, length);
        if ((uint16_t)crc_result != crc) {
            DBG_PRINT("%s: Invalid CRC received 0x%" PRIx16
                       " result of computation 0x%" PRIx16 "\r\n",
...
static uint8_t sd_write_block(sd_card_t *pSD, const uint8_t *buffer,
    ...
    if (!ret)
    {
        DBG_PRINT("Fail: sd_write_block sd_spi_transfer.\r\n");
    }
...
static int in_sd_write_blocks(sd_card_t *pSD, const uint8_t *buffer,
    ...
    if (blockCnt == 1) {
        ...
        if (response != SPI_DATA_ACCEPTED) {
            DBG_PRINT("Single Block Write failed: 0x%x \r\n", response);
            status = SD_BLOCK_DEVICE_ERROR_WRITE;
        }
    } else {
        ...
        do {
            ...
            if (response != SPI_DATA_ACCEPTED) {
                DBG_PRINT("Multiple Block Write failed: 0x%x\r\n", response);
                status = SD_BLOCK_DEVICE_ERROR_WRITE;
                break;
            }
            buffer += _block_size;
...
static int sd_init_medium(sd_card_t *pSD) {
    ...
    */
    ...
    if (sd_go_idle_state(pSD) != R1_IDLE_STATE) {
        DBG_PRINT("No disk, or could not put SD card in to SPI idle state\r\n");
    ...
    if ((SD_BLOCK_DEVICE_ERROR_NONE != status) || (0x00 != response)) {
        pSD->card_type = CARD_UNKNOWN;
        DBG_PRINT("Timeout waiting for card\r\n");
        return status;
    }
    ...
    if (SDCARD_V2 == pSD->card_type) {
        ...
        if (SD_BLOCK_DEVICE_ERROR_NONE ==
            ...
            if (response & OCR_HCS_CCS) {
                ...
                DBG_PRINT("Card Initialized: High Capacity Card\r\n");
            } else {
                DBG_PRINT(
        ...
        DBG_PRINT("Card Initialized: Version 1.x Card\r\n");
    }
...
static int sd_init(sd_card_t *pSD) {
    ...
    if (SD_BLOCK_DEVICE_ERROR_NONE != err) {
        DBG_PRINT("Failed to initialize card\r\n");
        sd_spi_release(pSD);
        sd_unlock(pSD);
        return pSD->m_Status;
    }
    DBG_PRINT("SD card initialized\r\n");
    ...
    if (sd_cmd(pSD, CMD16_SET_BLOCKLEN, _block_size, false, 0) != 0) {
        DBG_PRINT("Set %" PRIu32 "-byte block timed out\r\n", _block_size);
        sd_spi_release(pSD);
        sd_unlock(pSD);
        return pSD->m_Status;
    }
